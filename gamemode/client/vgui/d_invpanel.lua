...
function PANEL:Update()
	...
	if self._slot_old ~= self._slot or self._sort_old ~= self._sort then
		-- Grab the item number:
		local item = _2dm.Slot[self._slot]
...
function PANEL:SetSlot(slot, sort)
	-- Store the old slot:
	self._slot_old = self._slot
	self._sort_old = self._sort
...
function PANEL:Paint()
	...
	if self._sort == BP_WEP then
		if _2dm.Weapons[item] then	
			...
			-- Name of the weapon:
			local txt = _2dm.Weapons[item].Title
	...
	elseif self._sort == BP_GAD then
		if _2dm.gadgets[item] then	
			...
			-- Name of the weapon:
			local txt = _2dm.gadgets[item].name
			local w, h = surface.GetTextSize(txt)
			surface.SetTextPos(self.width/2 - w/2, self.width - h - 4)
			surface.DrawText(txt)
...
vgui.Register("D_invpanel", PANEL, "DModelPanel")
